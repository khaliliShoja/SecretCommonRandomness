# -*- coding: utf-8 -*-
"""
Created on Fri Jan  6 00:58:23 2017
...
for i in P:
    ...
    def Q_y_0(w,x):
        ...
        if(x==1):
            ...
            a_1=((P_Emission_y[0][0])/(P_Emission_y[1][0]))
            if(a_1<= z):
                a=a+P_Emission_y[0][0]
        
    
            a_4=((P_Emission_y[0][1])/(P_Emission_y[1][1]))
            if(a_4<= z):
                a=a+P_Emission_y[0][1]
    ...
    def Q_y_1(w,x):
        ...
        if(x==1):
            ...
            a_1=((P_Emission_y[0][0])/(P_Emission_y[1][0]))
            if(a_1<= z):
                a=a+P_Emission_y[1][0]
        
    
    
            a_4=((P_Emission_y[0][1])/(P_Emission_y[1][1]))
            if(a_4<= z):
                a=a+P_Emission_y[1][1]
    ...
    def der_Q_0(w,x):
        ...
        der=(Q_y_0(w,x+(2/delta))-Q_y_0(w,x))/(2/delta)     
        return der
        
    def der_Q_1(w,x):
        
        #der=(Q_xy_1(w,x+delta)-Q_xy_1(w,x-delta))/(2*delta)
        der=(Q_y_1(w,x+(2/delta))-Q_y_1(w,x))/(2/delta)
        return der    
    ...
    def G_0(w_i):
        ...
        for j in range (0,2):
                ...
                i_1=((1-alpha)*w_i + beta*(1-w_i)) *P_Emission_y[0][j]
                i_2=(alpha * w_i + (1-beta)*(1-w_i)) *P_Emission_y[1][j]
                S_G_0 = S_G_0 + (np.log10(i_1+i_2) * P_Emission_y[0][j])
    ...
    def G_1(w_i):
        ...
        for j in range (0,2):
                ...
                i_1=((1-alpha)*w_i + beta*(1-w_i)) * P_Emission_y[0][j]
                i_2=(alpha * w_i + (1-beta)*(1-w_i)) * P_Emission_y[1][j]
                S_G_1 = S_G_1 + (np.log10(i_1+i_2) * P_Emission_y[1][j])
